...
6. Stamps, video attached (\`gh pr comment <N> --attach <video.webm>\`): \`land: demo <head>\` for a pass (the gesture in three lines), \`land: demo failed <head>\` for a fail (the spec in a fenced block, expected against observed), \`land: demo skipped — needs a model\` / \`— stack: <what>\`. Each pass is then announced in #standards-logs as § 3 Demo point 5 says: the video converted to mp4 and uploaded through the session's Slack connector, playing inline, the announcement as its comment; no Slack tool in the session is noted in the note, not an error.
