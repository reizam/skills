...
| [`/pair`](skills/pair) | Builds a change live with the developer at the keyboard — codes the recommendation first, hands it over, takes feedback in conversation, commits each retained step as a checkpoint, and runs the build, the tests and `/evaluate` once, when the developer gives the go. |
